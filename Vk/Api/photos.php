...
trait Photos{
    ...
    public function upload_photo($file, $group = true){
        ...
        if($group)  $this->files_upload[] = $attach;
    ...
    public function upload_photo_wall($file, $group = false){
        if(preg_match("/^photo/imu", $file)){
        ...
        $upload_url = $this->{$method}("photos.getWallUploadServer", array("group_id"=>$this->group_id))['response']['upload_url'];
        ...
        $res = json_decode(curl_exec($ch), true);
        curl_close($ch);
        $attachment = $this->{$method}("photos.saveWallPhoto", array("group_id"=>$this->group_id, "photo"=>$res['photo'], "server"=>$res['server'], "hash"=>$res['hash']))['response'][0];
        $attach = "photo".$attachment['owner_id']."_".$attachment['id'];
